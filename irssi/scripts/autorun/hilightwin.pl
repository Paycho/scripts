...
sub sig_printtext {
  ...
  if (($dest->{level} & (MSGLEVEL_HILIGHT)) &&
    ...
    @args = ("notify-send", $dest->{target}.": ".$stripped);
    # logging out and logging in changes the DBUS address.  Best to just
    # run "unset DBUS_SESSION_BUS_ADDRESS" before starting your screen 
    # system(@args);
    # @args = ("wmmsg_notify","irc");
    system(@args);
