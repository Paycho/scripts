...
Irssi::theme_register([
	...
	'hilight_print_format' => '$0: %W<%n%M$1%n%W>%n :%W $2'
]);
			...
			if ($command =~ m{print})
                        {
                                ...
                                my $window = Irssi::window_find_name($cmd[1]);
                                $window->printformat(MSGLEVEL_CLIENTCRAP, 'hilight_print_format', $cmd[2],$cmd[3],$cmd[4]);
                                $recursion_depth--;
                                return;
                        }
